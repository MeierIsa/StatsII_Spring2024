...
# IM the coefficients are the same for the all the 3 models
# IM the odds of a male strongly agree with the statement is 0.56
...
summary(mult.log)
exp(coef(mult.log))
# which one to run? ordered or multinomial logistic regression. 
# IM In the multinomial the 3rd value is not suppose to be highest
...
# how do we interpret these coefficients?
# IM prestige doesn't seem to be very significant for example, so the ordered
# IM regression has given us the wrong idea
...
# IM for example the probability of disagree for the first line is 26%
